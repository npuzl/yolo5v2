...
cat 2007_train.txt 2007_val.txt >train.all.txt
...
print(os.path.exists('../tmp/2007_train.txt'))
with open('../tmp/2007_train.txt', 'r') as f:
...
print(os.path.exists('../tmp/2007_val.txt'))
with open('../tmp/2007_val.txt', 'r') as f:
    for line in f.readlines():
        line = "/".join(line.split('/')[-5:]).strip()
        if os.path.exists("../" + line):
